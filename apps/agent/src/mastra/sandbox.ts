...
export function createLocalSandbox(options: SandboxOptions): {
  sandbox: LocalSandbox
  dir: string
  isolation: ReturnType<typeof isolationFor>
} {
...
export function createSandboxTools(options: SandboxOptions) {
  const { sandbox, dir, isolation } = createLocalSandbox(options)
  const runCommand = createTool({
    id: 'run_command',
    description:
      `Run a shell command in an isolated scratch sandbox (${isolation} isolation, working dir ${dir}, network ${options.allowNetwork ? 'allowed' : 'blocked'}). ` +
  ...
  return { tools: { run_command: runCommand }, isolation, dir }
}
