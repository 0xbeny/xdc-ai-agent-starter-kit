...
describe('resolveSecrets', () => {
  ...
  const run = (file: string, args: string[]): string => {
    ...
    if (file === '/bin/sh') return 'OPENAI_API_KEY=helper-openai\nXAI_API_KEY=helper-xai\nSECRETS_COMMAND=ignored'
    throw new Error(`unexpected ${file}`)
  }

  it('resolves op:// and bws:// references and leaves plain values alone', () => {
    const out = resolveSecrets({ ANTHROPIC_API_KEY: 'op://vault/anthropic/key', MOONSHOT_API_KEY: 'bws://abc', OPENAI_API_KEY: 'plain' }, { run })
    ...
    expect(calls.some((c) => c.startsWith('op read --no-newline op://vault/anthropic/key'))).toBe(true)
  })
  ...
  it('reports failures without throwing', () => {
    const out = resolveSecrets({ A_TOKEN: 'op://x' }, { run: () => { throw new Error('op not installed') } })
...
describe('redaction', () => {
  it('finds credential-shaped keys and masks their values in text', () => {
    const env = { OPENAI_API_KEY: 'sk-live-1234567890', DASHBOARD_PASSWORD: 'hunter2hunter2', MODEL_CHAT: 'openai/gpt' } // gitleaks:allow — fake fixture
    expect(secretKeys(env).sort()).toEqual(['DASHBOARD_PASSWORD', 'OPENAI_API_KEY'])
    const redact = makeRedactor(env)
    expect(redact('key=sk-live-1234567890 pw=hunter2hunter2 model=openai/gpt')).toBe('key=***890 pw=***er2 model=openai/gpt') // gitleaks:allow
  })
