...
1. asks which provider to use — Anthropic, OpenAI, xAI (Grok), Moonshot (Kimi), Google, OpenRouter, Groq, DeepSeek, Ollama/local, **Claude Code or Codex CLI (personal subscription via the local CLI)**, or a custom OpenAI-compatible URL — then the model and the API key, and smoke-tests it;
2. optionally sets `fast` and `embed` slots;
3. links the XDC AI smart wallet with the OAuth **device-code grant** against `https://xdcai.tech` (RFC 8628; `api.xdcai.tech/mcp` advertises RFC 9728 protected-resource metadata), storing tokens in `data/xdcai-auth.json` (0600), then asks for the daily cap and auto-approve threshold;
