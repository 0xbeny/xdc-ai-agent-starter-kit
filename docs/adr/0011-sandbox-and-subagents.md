...
- **Sub-agents**: a supervisor pattern — the assistant delegates to specialised agents (research, treasury, ops) exposed as tools, each with its own instructions but the *same* payment policy and approval store. Background runs surface in the dashboard's Runs view; results post back into the originating thread.
